...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss /= N
        loss += self.reg * (np.sum(W1**2) + np.sum(W2**2))
        
        dsoftmax = scores
        dsoftmax[range(N), y] -= 1
        dsoftmax /= N
        dW2 = out1.T.dot(dsoftmax)
        db2 = dsoftmax.sum(axis=0)
        
        dout = dsoftmax.dot(W2.T)
        dX, dW1, db1 = affine_relu_backward(dout, cache1)
            
        dW1 +=  W1
        dW2 +=  W2
        
        grads = {'W1': dW1, 'W2': dW2, 'b1': b1, 'b2': db2}
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if(i!=self.num_layers-1):
                #affine 
                out, cache_aff = affine_forward(out, self.params[W], self.params[b])
                cache += [cache_aff,]
                #batch norm
                if(self.normalization=='batchnorm'):
                    gamma = 'gamma' + str(i+1)
                    beta = 'beta' + str(i+1)
                    out, cache_norm = batchnorm_forward(out,self.params[gamma], 
                                                        self.params[beta], self.bn_params[i])
                    cache += [cache_norm]
        ...
        dsoftmax /= N
        dout = dsoftmax
#         dx, dout = softmax_loss(scores, y)
        
        for i in range(self.num_layers, 0, -1):     
            W = 'W' + str(i)
            b = 'b' + str(i)
            if(i == self.num_layers):
                dx, grads[W], grads[b] = affine_backward(dout, caches[i-1])
                
            else: 
                if(self.normalization=='batchnorm' or self.use_dropout):
                    
                    if(self.use_dropout):
                            dx = dropout_backward(dx, caches[i-1].pop())

                    dx = relu_backward(dx, caches[i-1].pop())  

                    if(self.normalization=='batchnorm'):
                        gamma = 'gamma' + str(i)
                        beta = 'beta' + str(i)
                        dx, grads[gamma], grads[beta] = batchnorm_backward(dx, caches[i-1].pop())

                    dx, grads[W], grads[b] = affine_backward(dx, caches[i-1].pop())
