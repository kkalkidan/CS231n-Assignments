...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #Correct way to do derivative of softmax 
#         correct_scores = softmax[np.arange(N), y]
#         dsoftmax = np.multiply(softmax, correct_scores.reshape(N, 1))
#         dsoftmax[range(N), y] = np.multiply(correct_scores, (1-correct_scores))
        dsoftmax = softmax
        dsoftmax[np.arange(N) ,y] -= 1
        dsoftmax /= N
        
        dW2 = (Hidden.T).dot(dsoftmax)
        db2 = np.sum(dsoftmax, axis=0)
        
        dW1 = dsoftmax.dot(W2.T)
        dfunc1 = dW1 * (func1>0)
        dW1 = X.T.dot(dfunc1)  
        db1 = dfunc1.sum(axis=0)
    ...
    def predict(self, X):
        ...
        ###########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        func1 = X.dot(self.params['W1']) + self.params['b1']
        #RELU
        Hidden = func1
        Hidden[Hidden < 0] = 0
