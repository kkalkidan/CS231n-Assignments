...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if(self.cell_type == 'rnn'):            
            h, cache = rnn_forward(out_embed, out_affine, Wx, Wh, b)
        else :
            h, cache = lstm_forward(out_embed, out_affine, Wx, Wh, b)
        
        out_temp, cache_temp = temporal_affine_forward(h, W_vocab, b_vocab)
        loss, dx = temporal_softmax_loss(out_temp, captions_out, mask)
    ...
    def sample(self, features, max_length=30):
        ...
        h = out_affine
        c = np.zeros(h.shape)
        out_embed, cache_embed = word_embedding_forward(self._start, W_embed)
        V, W = W_embed.shape
        x = np.ones((N, W)) * out_embed
        ...
        for t in range(max_length):
            ...
            captions[:, t] = idx.reshape(idx.shape[0])
            
            x, cache_embed = word_embedding_forward(idx, W_embed)                      
